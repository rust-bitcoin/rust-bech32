    ...
    + ops::Neg
{
    ...
    /// Adds a value to `self`. This is a helper funcion for implementing the
    /// [`ops::Add`] and [`ops::AddAssign`] traits, which should probably be called
    /// instead of calling this.
    fn _add(&self, other: &Self) -> Self;

    /// Subtracts a value from `self`. This is a helper funcion for implementing the
    /// [`ops::Sub`] and [`ops::SubAssign`] traits, which should probably be called
    /// instead of calling this.
    fn _sub(&self, other: &Self) -> Self;

    /// Multiplies a value by `self`. This is a helper funcion for implementing the
