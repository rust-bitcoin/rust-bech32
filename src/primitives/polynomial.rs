...
impl<F: Field> Polynomial<F> {
    ...
    pub fn with_monic_leading_term(coeffs: &[F]) -> Self {
        Polynomial { inner: iter::once(F::ONE).chain(coeffs.iter().cloned()).collect() }
    }
    ...
    pub fn find_nonzero_distinct_roots<E: ExtensionField<BaseField = F>>(
        &self,
        base: E,
    ) -> RootIter<E> {
