...
impl<F: Field> LfsrIter<F> {
    ...
    ///
    /// If the initial contents are empty, outputs the constant zero.
    ///
    /// # Panics
    ///
    /// Panics on non-`alloc` builds if the list of initial contents is greater than or
    /// equal to `NO_ALLOC_MAX_LENGTH`.
    pub fn berlekamp_massey(initial_contents: &[F]) -> Self {
        ...
        if conn.len() == 1 {
            // We may wind up with empty contents; this happens if `initial_contents`
            // contained nothing but zeroes, which can be generated by the empty LFSR.
            // In this case we manually construct a LFSR with a single explicit zero
            // coefficient, since `LsfrIter::next` assumes that there's always something
            // in `contents`.
            let contents = Some(F::ZERO).into_iter().collect();
            conn.push(F::ZERO);
            Self { contents, coeffs: conn.into() }
        } else {
            // Otherwise, copy conn.len() (less the monic term) initial elements into the LSFR.
