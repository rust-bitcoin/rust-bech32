...
pub trait Fe32IterExt: Sized + ExactSizeIterator + FusedIterator + Iterator<Item = Fe32> {
    ...
    /// Adapt the Fe32 iterator by prepending a witness version.
    fn with_witness_version(self, witness_version: WitnessVersion) -> WitnessVersionIter<Self> {
        WitnessVersionIter { witness_version: Some(Fe32(witness_version.to_num())), iter: self }
    }

    /// Adapts the Fe32 iterator to append a checksum to the end of the data.
    ///
    /// Because the HRP of a bech32 string needs to be expanded before being checksummed, this
    /// iterator is a little bit inconvenient to use on raw data. The
    /// [`ChecksumIter::with_checksummed_hrp`] method may be of use.
    ...
    /// Adapts the Fe32 iterator to output characters including the human-readable part.
    fn hrp_char(self, hrp: Hrp<'_>) -> HrpCharIter<'_, Self> {
        HrpCharIter { hrp_iter: hrp.iter(), hrp_done: false, sep_done: false, fe_iter: self }
    }
}
...
///
/// More ergonomic to use than `std::iter::once(fe).chain(iter)` and also checks
/// on construction that the given witness version is a legal witness version.
...
where
    I: ExactSizeIterator + FusedIterator + Iterator<Item = Fe32>,
{
    hrp_iter: hrp::LowercaseIter<'hrp>,
    hrp_done: bool,
    sep_done: bool,
    fe_iter: I,
}
